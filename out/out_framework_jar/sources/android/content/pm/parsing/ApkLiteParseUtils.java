...
public class ApkLiteParseUtils {
    ...
    /* JADX DEBUG: Don't trust debug lines info. Repeating lines: [226=6] */
    ...
    */
    public static ParseResult<PackageLite> parseClusterPackageLite(ParseInput input, File packageDirOrApk, List<File> frameworkSplits, int flags) {
        ...
        int i = 0;
        int i2 = 1;
        boolean parsingFrameworkSplits = (flags & 256) != 0;
        if (!parsingFrameworkSplits) {
            File[] files2 = packageDirOrApk.listFiles();
            if (ArrayUtils.isEmpty(files2)) {
                return input.error(-100, "No packages found in split");
            }
            if (files2.length == 1 && files2[0].isDirectory()) {
                return parseClusterPackageLite(input, files2[0], frameworkSplits, flags);
            }
            files = files2;
        } else if (ArrayUtils.isEmpty(frameworkSplits)) {
            return input.error(-100, "No packages found in split");
        } else {
            files = (File[]) frameworkSplits.toArray(new File[frameworkSplits.size() + 1]);
            files[files.length - 1] = packageDirOrApk;
        }
        int flags2 = parsingFrameworkSplits ? flags & (-33) : flags;
        ...
        try {
            ...
            while (true) {
                ...
                File file = files[i];
                if (isApkFile(file)) {
                    ParseResult<ApkLite> result = parseApkLite(input, file, flags2);
                    if (result.isError()) {
                    ...
                    if (packageName == null) {
                        ...
                        ParseResult<PackageLite> error2 = input.error(-101, "Inconsistent package " + lite.getPackageName() + " in " + file + "; expected " + packageName);
                        Trace.traceEnd(262144L);
                        return error2;
                    } else if (!parsingFrameworkSplits && versionCode != lite.getVersionCode()) {
                        ParseResult<PackageLite> error3 = input.error(-101, "Inconsistent version " + lite.getVersionCode() + " in " + file + "; expected " + versionCode);
                        Trace.traceEnd(262144L);
                        return error3;
                    }
                    if (apks.put(lite.getSplitName(), lite) != null) {
                        ParseResult<PackageLite> error4 = input.error(-101, "Split name " + lite.getSplitName() + " defined more than once; most recent was " + file);
                        Trace.traceEnd(262144L);
                        return error4;
                    }
                }
    ...
    /* JADX DEBUG: Don't trust debug lines info. Repeating lines: [386=4, 391=8, 392=5, 394=5, 395=5, 396=5] */
    ...
    /* JADX WARN: Code restructure failed: missing block: B:113:0x035d, code lost:
        if (android.content.pm.parsing.FrameworkParsingPackageUtils.checkRequiredSystemProperties(r14, r11) != false) goto L25;
     */
