...
public abstract class ContentProvider implements ContentInterface, ComponentCallbacks2 {
    ...
    private boolean isClonedAppQueryingMediaFile(Uri uri) {
        return Binder.getCallingUserHandle().equals(UserHandle.DUAL_PROFILE) && uri != null && DeviceConfig.NAMESPACE_MEDIA.equals(uri.getAuthority());
    }
    ...
    boolean checkUser(int pid, int uid, Context context) {
        ...
        if (callingUserId == context.getUserId() || this.mSingleUser || UserHandle.getUserId(uid) == 999 || context.checkPermission(Manifest.permission.INTERACT_ACROSS_USERS, pid, uid) == 0 || context.checkPermission(Manifest.permission.INTERACT_ACROSS_USERS_FULL, pid, uid) == 0) {
            return true;
        }
    ...
    public int enforceReadPermissionInner(Uri uri, AttributionSource attributionSource) throws SecurityException {
        ...
        if (context.checkUriPermission(userUri, pid, uid, 1) != 0) {
            ...
            if ("android".equals(attributionSource.getPackageName()) || "com.transsion.resolver".equals(attributionSource.getPackageName())) {
                return 0;
            }
    ...
    public Uri validateIncomingUri(Uri uri) throws SecurityException {
        ...
        if (!this.mSingleUser && (userId = getUserIdFromAuthority(auth, -2)) != -2 && userId != this.mContext.getUserId() && userId != 999) {
