...
public class ScrollView extends FrameLayout {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (getScrollY() != 0 || canScrollVertically(1) || this.mTranBounceEffectSupport) {
            switch (action & 255) {
                case 0:
                    int y = (int) ev.getY();
        ...
        if (this.mScroller.computeScrollOffset()) {
            ...
            if (oldX != x || oldY != y) {
                ...
                if (canOverscroll) {
                    ...
                    if (this.mTranBounceEffectSupport && ((y < 0 && oldY >= 0) || (y > range && oldY <= range))) {
                        this.mScroller.abortAnimation();
                        StrictMode.Span span = this.mFlingStrictSpan;
                        if (span != null) {
                            span.finish();
    ...
    public void fling(int velocityY) {
        if (getChildCount() > 0) {
            int height = (getHeight() - this.mPaddingBottom) - this.mPaddingTop;
            int bottom = getChildAt(0).getHeight();
            if (this.mTranBounceEffectSupport) {
                this.mScroller.fling(this.mScrollX, this.mScrollY, 0, velocityY, 0, Integer.MAX_VALUE, 0, Integer.MAX_VALUE);
            } else {
