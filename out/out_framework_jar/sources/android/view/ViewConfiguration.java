...
public class ViewConfiguration {
    ...
    private ViewConfiguration(Context context) {
        ...
        if (SystemProperties.get("vendor.transsion.scroll_adjust.support").equals("1")) {
            this.mMaximumFlingVelocity = Integer.valueOf(SystemProperties.get("vendor.transsion.max_fling_velocity")).intValue();
        } else {
    ...
    public int getScaledTouchSlop() {
        if (ITranView.Instance().isAppturboSupport()) {
            return ITranView.Instance().getScaledTouchSlop(this.mTouchSlop);
        }
    ...
    public static float getScrollFriction() {
        if (!SystemProperties.get("vendor.transsion.scroll_adjust.support").equals("1") || !SystemProperties.get("vendor.transsion.enable_supper_touch").equals(AudioSystem.LEGACY_REMOTE_SUBMIX_ADDRESS)) {
            return ITranView.Instance().isAppturboSupport() ? ITranView.Instance().getScrollFriction(SCROLL_FRICTION) : SCROLL_FRICTION;
        }
        float SCROLL_FRICTION_SPD = Float.valueOf(SystemProperties.get("vendor.transsion.scroll_friction")).floatValue();
        return SCROLL_FRICTION_SPD;
    }
