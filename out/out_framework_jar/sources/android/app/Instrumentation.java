...
public class Instrumentation {
    ...
    private static final String TRAN_SPLITSCREEN_ACTIVITY_MONITOR = "androidx.window.extensions.embedding.SplitController";
    ...
    */
    public ActivityResult execStartActivity(Context who, IBinder contextThread, IBinder token, Activity target, Intent intent, int requestCode, Bundle options) {
        ...
        try {
            ITranActivityThread.Instance().generateApkInfoForSUN(intent, who);
    ...
    */
    public ActivityResult execStartActivity(Context who, IBinder contextThread, IBinder token, String target, Intent intent, int requestCode, Bundle options) {
        ...
        try {
            ...
            intent.migrateExtraStreamToClipData(who);
            intent.prepareToLeaveProcess(who);
            int result2 = ActivityTaskManager.getService().startActivity(whoThread, who.getOpPackageName(), who.getAttributionTag(), intent, intent.resolveTypeIfNeeded(who.getContentResolver()), token, target, requestCode, 0, null, options2);
            checkStartActivityResult(result2, intent);
            return null;
        } catch (RemoteException e) {
            throw new RuntimeException("Failure from system", e);
    ...
    */
    public ActivityResult execStartActivity(Context who, IBinder contextThread, IBinder token, String resultWho, Intent intent, int requestCode, Bundle options, UserHandle user) {
        ...
        try {
            ...
            intent.prepareToLeaveProcess(who);
            try {
                int result2 = ActivityTaskManager.getService().startActivityAsUser(whoThread, who.getOpPackageName(), who.getAttributionTag(), intent, intent.resolveTypeIfNeeded(who.getContentResolver()), token, resultWho, requestCode, 0, null, options2, user.getIdentifier());
                checkStartActivityResult(result2, intent);
                return null;
            } catch (RemoteException e) {
                e = e;
    ...
    */
    public ActivityResult execStartActivityAsCaller(Context who, IBinder contextThread, IBinder token, Activity target, Intent intent, int requestCode, Bundle options, boolean ignoreTargetSecurity, int userId) {
        ...
        try {
            ...
            intent.prepareToLeaveProcess(who);
            try {
                int result2 = ActivityTaskManager.getService().startActivityAsCaller(whoThread, who.getOpPackageName(), intent, intent.resolveTypeIfNeeded(who.getContentResolver()), token, target != null ? target.mEmbeddedID : null, requestCode, 0, null, options2, ignoreTargetSecurity, userId);
                try {
                    checkStartActivityResult(result2, intent);
                    return null;
                } catch (RemoteException e) {
    ...
    private boolean isInMultiwindow(Context context, ActivityMonitor am) {
        if (am == null || context == null || context.getResources() == null || context.getResources().getConfiguration() == null || context.getResources().getConfiguration().windowConfiguration == null) {
            return false;
        }
        boolean isInMultiwindow = context.getResources().getConfiguration().windowConfiguration.isThunderbackWindow();
        return isInMultiwindow && am.getClass().getName().contains(TRAN_SPLITSCREEN_ACTIVITY_MONITOR);
    }
}
