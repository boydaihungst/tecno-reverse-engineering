...
public class Dialog implements DialogInterface, Window.Callback, KeyEvent.Callback, View.OnCreateContextMenuListener, Window.OnWindowDismissedCallback {
    ...
    private static final int OS_DIALOG_BOTTOM_DISTANCE = 56;
    ...
    public Dialog(Context context, int themeResId, boolean createContextThemeWrapper) {
        ...
        w.setOnWindowSwipeDismissedCallback(new Window.OnWindowSwipeDismissedCallback() { // from class: android.app.Dialog$$ExternalSyntheticLambda1
            ...
            public final void onWindowSwipeDismissed() {
                Dialog.this.m266lambda$new$0$androidappDialog();
            }
        });
        w.setWindowManager(windowManager, null, null);
        TypedArray ta = this.mContext.obtainStyledAttributes(R.styleable.OSWindow);
        boolean isPortrait = context.getResources().getConfiguration().orientation == 1;
        if (!isOriginalDialog(context) || !isPortrait) {
            ...
            lp.y = dp2px(56) - this.mContext.getResources().getDimensionPixelSize(com.android.internal.R.dimen.navigation_bar_height);
            w.setAttributes(lp);
        }
        ta.recycle();
    ...
    public /* synthetic */ void m266lambda$new$0$androidappDialog() {
        if (this.mCancelable) {
            cancel();
        }
    }
    ...
    private int dp2px(int dpValues) {
        return (int) TypedValue.applyDimension(1, dpValues, this.mContext.getResources().getDisplayMetrics());
    }

    private boolean isOriginalDialog(Context context) {
        Bundle bundle;
        try {
            ...
            boolean z = false;
            if (bundle.getBoolean("dialog_on_bottom", false)) {
                if (!"com.transsion.widgetslib.dialog.PromptDialog".equals(getClass().getName())) {
                    z = true;
                }
            }
            boolean isOriginalDialog = z;
            return isOriginalDialog;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
