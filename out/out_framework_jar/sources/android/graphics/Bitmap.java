...
public final class Bitmap implements Parcelable {
    ...
    private static native Bitmap nativeCopyFingerprintDim(long j, int i, boolean z);
    ...
    public static Bitmap createFingerprintDimBitmap(Bitmap src, int dstWidth, int dstHeight, boolean filter) {
        Matrix m = new Matrix();
        int width = src.getWidth();
        int height = src.getHeight();
        if (width != dstWidth || height != dstHeight) {
        ...
        return createFingerprintBitmap(src, 0, 0, width, height, m, filter);
    }

    public static Bitmap createFingerprintBitmap(Bitmap source, int x, int y, int width, int height, Matrix m, boolean filter) {
        Config newConfig;
        RectF deviceR;
        Bitmap bitmap;
        Paint paint;
        ...
        if (isHardware) {
            return bitmap.copyFingerprintDim(Config.HARDWARE, false);
        }
        return bitmap;
    }
    ...
    public Bitmap copyFingerprintDim(Config config, boolean isMutable) {
        checkRecycled("Can't copy a recycled bitmap");
        if (config == Config.HARDWARE && isMutable) {
            throw new IllegalArgumentException("Hardware bitmaps are always immutable");
        }
        noteHardwareBitmapSlowCall();
        Bitmap b = nativeCopyFingerprintDim(this.mNativePtr, config.nativeInt, isMutable);
        if (b != null) {
            b.setPremultiplied(this.mRequestPremultiplied);
            b.mDensity = this.mDensity;
        }
