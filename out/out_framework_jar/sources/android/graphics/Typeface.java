...
import com.transsion.hubcore.graphics.ITranGraphic;
...
public class Typeface {
    ...
    public static Typeface create(Typeface family, int style) {
        ...
        synchronized (sStyledCacheLock) {
            ITranGraphic Instance = ITranGraphic.Instance();
            LongSparseArray<SparseArray<Typeface>> longSparseArray = sStyledTypefaceCache;
            Typeface typeface = Instance.onCreateStyleTypeface(longSparseArray, family, style);
            if (typeface != null) {
                return typeface;
            }
            ...
            if (styles == null) {
                ...
                Typeface typeface2 = styles.get(style);
                if (typeface2 != null) {
                    return typeface2;
                }
            }
            Typeface typeface3 = new Typeface(nativeCreateFromTypeface(ni, style));
            styles.put(style, typeface3);
            return typeface3;
        }
    }
    ...
    public static Typeface createWeightStyle(Typeface base, int weight, boolean italic) {
        ...
        synchronized (sWeightCacheLock) {
            ...
            Typeface typeface = Instance.onCreateWeightStyleTypeface(longSparseArray, base, key, weight, italic);
            if (typeface != null) {
                return typeface;
            }
            ...
            Typeface typeface3 = new Typeface(nativeCreateFromTypefaceWithExactStyle(base.native_instance, weight, italic));
            innerCache.put(key, typeface3);
            return typeface3;
        }
    }
    ...
    public static void setSystemFontMap(Map<String, Typeface> systemFontMap) {
        synchronized (SYSTEM_FONT_MAP_LOCK) {
            ...
            ITranGraphic.Instance().onInitSystemDefaultTypefaces(map);
    ...
    public static void dump(PrintWriter pw) {
        pw.println();
        pw.println("Typeface");
        StringBuilder append = new StringBuilder().append(" System:");
        Map<String, Typeface> map = sSystemFontMap;
        pw.println(append.append(map.size()).toString());
        LongSparseArray<String> sSystemNames = new LongSparseArray<>();
        for (Map.Entry<String, Typeface> entry : map.entrySet()) {
            String name = entry.getKey();
            Typeface tf = entry.getValue();
            sSystemNames.put(tf.native_instance, name);
            pw.println("  family name:" + name + " typeface:" + tf.toShortString());
        }
        StringBuilder append2 = new StringBuilder().append(" Fallback:");
        Map<String, FontFamily[]> map2 = sSystemFallbackMap;
        pw.println(append2.append(map2.size()).toString());
        for (Map.Entry<String, FontFamily[]> entry2 : map2.entrySet()) {
            pw.println("  fallback family name:" + entry2.getKey() + " size:" + entry2.getValue().length);
        }
        synchronized (sStyledCacheLock) {
            pw.println(" StyleCache:" + sStyledTypefaceCache.size());
            int i = 0;
            while (true) {
                LongSparseArray<SparseArray<Typeface>> longSparseArray = sStyledTypefaceCache;
                ...
                long ni = longSparseArray.keyAt(i);
                SparseArray<Typeface> styles = longSparseArray.valueAt(i);
                pw.println("  base ni:" + Long.toHexString(ni));
                for (int j = 0; j < styles.size(); j++) {
                    int style = styles.keyAt(j);
                    Typeface tf2 = styles.valueAt(j);
                    pw.println("   style:" + style + " typeface:" + tf2.toShortString());
                }
                i++;
            }
        }
        synchronized (sWeightCacheLock) {
            pw.println(" WeightCache:" + sWeightTypefaceCache.size());
            int i2 = 0;
            while (true) {
                LongSparseArray<SparseArray<Typeface>> longSparseArray2 = sWeightTypefaceCache;
                if (i2 >= longSparseArray2.size()) {
                    break;
                }
                long ni2 = longSparseArray2.keyAt(i2);
                SparseArray<Typeface> styles2 = longSparseArray2.valueAt(i2);
                pw.println("  base ni:" + Long.toHexString(ni2));
                for (int j2 = 0; j2 < styles2.size(); j2++) {
                    int key = styles2.keyAt(j2);
                    int weight = key >> 1;
                    boolean z = true;
                    if ((key & 1) != 1) {
                        z = false;
                    ...
                    boolean italic = z;
                    Typeface tf3 = styles2.valueAt(j2);
                    pw.println("   weight:" + weight + " italic:" + italic + " typeface:" + tf3.toShortString());
                }
                i2++;
            }
        }
        synchronized (sDynamicCacheLock) {
            StringBuilder append3 = new StringBuilder().append(" DynamicCache:");
            LruCache<String, Typeface> lruCache = sDynamicTypefaceCache;
            pw.println(append3.append(lruCache.size()).toString());
            Map<String, Typeface> map3 = lruCache.snapshot();
            for (Map.Entry<String, Typeface> entry3 : map3.entrySet()) {
                Typeface typeface = entry3.getValue();
                pw.println("  name:" + entry3.getKey());
                pw.println("  typeface:" + typeface.toShortString());
            }
        }
        ITranGraphic.Instance().dumpTypeface(pw);
    }

    private String toShortString() {
        return "Typeface@" + Integer.toHexString(hashCode()) + " ni:" + Long.toHexString(this.native_instance) + " mStyle:" + getStyle() + " mWeight:" + getWeight();
    }
}
