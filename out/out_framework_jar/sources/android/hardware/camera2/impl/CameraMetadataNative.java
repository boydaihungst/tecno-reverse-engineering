...
public class CameraMetadataNative implements Parcelable {
    ...
    private static final boolean TRAN_VIDEOCALL_POWERSAVE_SUPPORT = SystemProperties.get("persist.tran.videocallpowersave.support").equals("1");
    private static final boolean TRAN_VIDEOCALL_POWERSAVE_CK6_ONLY = SystemProperties.get("persist.tran.videocallpowersave.ck6").equals("1");
    private static final String[] mTranVideocallActivityList = {"com.whatsapp.voipcalling.VoipActivityV2", "com.instagram.rtc.activity.RtcCallActivity", "com.google.android.libraries.communications.conference.ui.callui.CallActivity", "com.google.android.libraries.communications.conference.ui.callui.SingleCallActivity", "com.cisco.webex.meetings.ui.inmeeting.MeetingClient"};
    private static final String[] mTranVideocallAppList = {"com.whatsapp", "com.fmwhatsapp", "com.blueWAplus", "com.instagram.android", "com.google.android.apps.meetings", "com.cisco.webex.meetings"};
    ...
    public <T> void set(CaptureRequest.Key<T> key, T value) {
        if (key == CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE && TRAN_VIDEOCALL_POWERSAVE_SUPPORT) {
            Range<Integer> oldFpsRange = (Range) value;
            Range<Integer> newFpsRange = Range.create(5, 20);
            if (oldFpsRange != null && newFpsRange != null && oldFpsRange.getUpper().intValue() > newFpsRange.getUpper().intValue()) {
                String appName = ActivityThread.currentOpPackageName();
                if (Arrays.asList(mTranVideocallAppList).contains(appName)) {
                    try {
                        IActivityManager am = ActivityManager.getService();
                        List<ActivityManager.RunningTaskInfo> runningTasks = am.getTasks(1);
                        if (runningTasks != null && !runningTasks.isEmpty() && runningTasks.get(0).topActivity != null) {
                            String topact = runningTasks.get(0).topActivity.getClassName();
                            if (Arrays.asList(mTranVideocallActivityList).contains(topact)) {
                                Log.d(TAG, "VideoCallPowerSave[DEBUG]: Activity name -> " + topact);
                                if (TRAN_VIDEOCALL_POWERSAVE_CK6_ONLY && topact.equals("com.whatsapp.voipcalling.VoipActivityV2")) {
                                    newFpsRange = Range.create(5, 25);
                                }
                                Range<Integer> newFpsRange2 = newFpsRange.intersect(oldFpsRange);
                                set((Key<Key<T>>) key.getNativeKey(), (Key<T>) newFpsRange2);
                                Log.d(TAG, "VideoCallPowerSave[DEBUG]: oldFpsRange: " + oldFpsRange + ",newFpsRange:" + newFpsRange2);
                                return;
                            }
                        }
                    } catch (RemoteException e) {
                        Log.e(TAG, "VideoCallPowerSave get activity failed!");
                    }
                }
            }
        }
    ...
    public StreamConfigurationMap getStreamConfigurationMap() {
        ...
        StreamConfiguration[] configurations2 = ITranCameraMetadataNative.Instance().getStreamConfigurationMap(ActivityThread.currentOpPackageName(), configurations, new ITranCameraMetadataNative.ICallback() { // from class: android.hardware.camera2.impl.CameraMetadataNative$$ExternalSyntheticLambda0
            @Override // com.transsion.hubcore.hardware.camera2.impl.ITranCameraMetadataNative.ICallback
            public final StreamConfiguration[] getBase(CameraCharacteristics.Key key) {
                return CameraMetadataNative.this.m1464x23747faa(key);
            }
        });
        ...
        boolean listHighResolution = isBurstSupported();
        return new StreamConfigurationMap(configurations2, minFrameDurations, stallDurations, depthConfigurations, depthMinFrameDurations, depthStallDurations, dynamicDepthConfigurations, dynamicDepthMinFrameDurations, dynamicDepthStallDurations, heicConfigurations, heicMinFrameDurations, heicStallDurations, highSpeedVideoConfigurations, inputOutputFormatsMap, listHighResolution);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* renamed from: lambda$getStreamConfigurationMap$0$android-hardware-camera2-impl-CameraMetadataNative  reason: not valid java name */
    public /* synthetic */ StreamConfiguration[] m1464x23747faa(CameraCharacteristics.Key key) {
        return (StreamConfiguration[]) getBase(key);
    }
