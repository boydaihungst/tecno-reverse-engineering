...
public class Camera {
    ...
    private static final String TAG_SM = "SMaster";
    private static final boolean TRAN_SLEEP_MASTER_SUPPORT = "1".equals(SystemProperties.get("ro.sleep_master"));
        ...
        try {
            ...
            if (!runningTasks.isEmpty() && runningTasks.get(0).topActivity != null) {
                ...
                while (true) {
                    ...
                    int[] range = it.next();
                    if (range[0] == 5000 && range[1] == 20000) {
                        fpsFlag = true;
                        break;
                    }
                }
                if (fpsFlag && ((topact.contains("imo") && topact.contains("av")) || ((topact.contains("imo") && topact.contains("activities")) || topact.contains("com.android.launcher3.Launcher") || ((topact.contains("whatsapp") && topact.contains("voipcalling")) || topact.contains("com.zipow.videobox.ConfActivityNormal") || (topact.contains("facebook") && topact.contains("WebrtcIncall")))))) {
                    params.setPreviewFpsRange(5000, 20000);
                    Log.d(TAG, "VideoCallPowerSave,CAMERA_POWERSAVE_DISABLED 0");
                } else if (topact.contains("imo") || topact.contains("whatsapp") || topact.contains("facebook.katana") || topact.contains("facebook.orca") || topact.contains("zipow.videobox")) {
                    params.setPreviewFpsRange(5000, 30000);
                    Log.d(TAG, "VideoCallPowerSave,CAMERA_POWERSAVE_DISABLED 1");
                }
            }
        } catch (RemoteException e) {
    ...
    public class Parameters {
        ...
        public void setFlashMode(String value) {
            ...
            if (Camera.TRAN_SLEEP_MASTER_SUPPORT) {
                IPowerManager pm = IPowerManager.Stub.asInterface(ServiceManager.getService(Context.POWER_SERVICE));
                try {
                    if (FLASH_MODE_TORCH.equals(value)) {
                        pm.setTorch(true);
                    } else {
                        pm.setTorch(false);
                    }
                    Log.d(Camera.TAG_SM, " setFlashMode " + value);
                } catch (RemoteException e) {
                    Log.d(Camera.TAG_SM, " setFlashMode. " + e);
                }
            }
        }
