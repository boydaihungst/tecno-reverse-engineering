...
public final class ProcessState {
    ...
    public void setCombinedState(int state, long now) {
        ...
        if (!this.mDead && this.mCurCombinedState != state) {
            ...
            UidState nStates = this.mStats.mUidStates.get(this.mUid);
            if (nStates != null) {
                nStates.updateCombinedState(state, now);
            }
        }
    }
    ...
    public void dumpStateDurationToStatsd(int atomTag, ProcessStats processStats, StatsEventOutput statsEventOutput) {
        ...
        int seconds = (int) TimeUnit.MILLISECONDS.toSeconds(cachedMs2);
        long cachedMs4 = TimeUnit.MILLISECONDS.toSeconds(0L);
        statsEventOutput.write(atomTag, uid, name, (int) timeUnit.toSeconds(otherMs2), (int) timeUnit2.toSeconds(cachedMs3), (int) TimeUnit.MILLISECONDS.toSeconds(processStats.mTimePeriodEndUptime - processStats.mTimePeriodStartUptime), (int) TimeUnit.MILLISECONDS.toSeconds(topMs), (int) TimeUnit.MILLISECONDS.toSeconds(fgsMs), (int) TimeUnit.MILLISECONDS.toSeconds(boundTopMs), (int) topMs2, (int) TimeUnit.MILLISECONDS.toSeconds(importantForegroundMs), seconds, (int) cachedMs4, (int) TimeUnit.MILLISECONDS.toSeconds(otherMs));
    }
