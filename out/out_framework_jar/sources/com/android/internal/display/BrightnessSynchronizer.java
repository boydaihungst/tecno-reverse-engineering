...
public class BrightnessSynchronizer {
    ...
    private static float brightnessFloatToIntRange(float brightnessFloat, float minFloat, float maxFloat, float minInt, float maxInt) {
        if (floatEquals(brightnessFloat, -1.0f)) {
            return 0.0f;
        }
        if (Float.isNaN(brightnessFloat)) {
    ...
    public static int brightnessFloatTo4095Int(Context context, float brightnessFloat) {
        ...
        return Math.round(brightnessFloatToIntRange(brightnessFloat, 0.0f, 1.0f, pmMinBrightness, 4095.0f));
    }
}
