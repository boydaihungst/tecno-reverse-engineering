...
public class NotificationActionListLayout extends LinearLayout {
    ...
    private int measureChildAndGetUsedWidth(int widthMeasureSpec, int heightMeasureSpec, int innerWidth, boolean collapsePriorityActions) {
        View c;
        boolean isPriority;
        ...
        boolean constrained = View.MeasureSpec.getMode(widthMeasureSpec) != 0;
        int otherSize = this.mMeasureOrderOther.size();
        int avgWidth = innerWidth / this.mNumNotGoneChildren;
        int usedWidth = 0;
        int measuredChildren = 0;
        int measuredPriorityChildren = 0;
        int i2 = 0;
        while (i2 < numChildren) {
            ...
            if (c.getVisibility() == 8) {
                ...
                ViewGroup.MarginLayoutParams lp2 = (ViewGroup.MarginLayoutParams) c.getLayoutParams();
                int availableWidth = innerWidth - usedWidth;
                int i3 = this.mNumNotGoneChildren;
                int unmeasuredChildren = i3 - measuredChildren;
                int maxWidthForChild = availableWidth / unmeasuredChildren;
                if (constrained && isPriority) {
                    int unmeasuredPriorityChildren = this.mNumPriorityChildren - measuredPriorityChildren;
                    int unmeasuredOtherChildren2 = unmeasuredChildren - unmeasuredPriorityChildren;
                    int widthReservedForOtherChildren = (innerWidth * unmeasuredOtherChildren2) / 4;
                    int widthAvailableForPriority = availableWidth - widthReservedForOtherChildren;
                    int maxWidthForChild2 = widthAvailableForPriority / unmeasuredPriorityChildren;
                    unmeasuredOtherChildren = maxWidthForChild2;
                } else {
                    unmeasuredOtherChildren = maxWidthForChild;
                }
                int usedWidthForChild = innerWidth - unmeasuredOtherChildren;
                if (i3 != 1 || c.getMeasuredWidth() < avgWidth) {
                    lp = lp2;
                    i = i2;
                    if (c.getMeasuredWidth() > avgWidth) {
                        measureChildWithMargins(c, widthMeasureSpec, avgWidth, heightMeasureSpec, 0);
                    } else {
                        measureChildWithMargins(c, widthMeasureSpec, usedWidthForChild, heightMeasureSpec, 0);
                    }
                } else {
                    lp = lp2;
                    i = i2;
                    measureChildWithMargins(c, widthMeasureSpec, (avgWidth / 5) * 4, heightMeasureSpec, 0);
                }
                usedWidth += c.getMeasuredWidth() + lp.rightMargin + lp.leftMargin;
                measuredChildren++;
                if (isPriority) {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int usedWidth = measureChildAndGetUsedWidth(widthMeasureSpec, heightMeasureSpec, innerWidth, false);
