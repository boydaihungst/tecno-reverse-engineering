...
import com.transsion.hubcore.server.connectivity.ITranNetdEventListenerService;
...
public class NetdEventListenerService extends INetdEventListener.Stub {
    ...
    public NetdEventListenerService(Context context) {
        ...
        ITranNetdEventListenerService.Instance().create(context);
    }
    ...
    public NetdEventListenerService(ConnectivityManager cm) {
        ...
        this.mCm = cm;
        cm.registerNetworkCallback(new NetworkRequest.Builder().clearCapabilities().build(), transportForNetIdNetworkCallback);
    }
    ...
    private NetworkMetrics getMetricsForNetwork(long timeMs, int netId) {
        ...
        NetworkCapabilities nc = this.mCallback.getNetworkCapabilities(netId);
        long transports = nc != null ? BitUtils.packBits(nc.getTransportTypes()) : 0L;
        ...
        if (metrics == null || forceCollect) {
            NetworkMetrics metrics2 = new NetworkMetrics(netId, transports, this.mConnectTb);
    ...
    private void collectPendingMetricsSnapshot(long timeMs, boolean forceCollect) {
        if (!forceCollect && Math.abs(timeMs - this.mLastSnapshot) <= 300000) {
            return;
        }
    ...
    public synchronized void onDnsEvent(int netId, int eventType, int returnCode, int latencyMs, String hostname, String[] ipAddresses, int ipAddressesCount, int uid) throws RemoteException {
        ...
        ITranNetdEventListenerService.Instance().onDnsEvent(netId, eventType, returnCode, latencyMs, hostname, ipAddresses, ipAddressesCount, uid);
    }
    ...
    public synchronized void onNat64PrefixEvent(int netId, boolean added, String prefixString, int prefixLength) throws RemoteException {
        ...
        ITranNetdEventListenerService.Instance().onNat64PrefixEvent(netId, added, prefixString, prefixLength);
    }
    ...
    public synchronized void onPrivateDnsValidationEvent(int netId, String ipAddress, String hostname, boolean validated) throws RemoteException {
        ...
        ITranNetdEventListenerService.Instance().onPrivateDnsValidationEvent(netId, ipAddress, hostname, validated);
    }
    ...
    public synchronized void onConnectEvent(int netId, int error, int latencyMs, String ipAddr, int port, int uid) throws RemoteException {
        ...
        ITranNetdEventListenerService.Instance().onConnectEvent(netId, error, latencyMs, ipAddr, port, uid);
    }
    ...
    public synchronized void onWakeupEvent(String prefix, int uid, int ethertype, int ipNextHeader, byte[] dstHw, String srcIp, String dstIp, int srcPort, int dstPort, long timestampNs) {
        ...
        ITranNetdEventListenerService.Instance().onWakeupEvent(prefix, uid, ethertype, ipNextHeader, dstHw, srcIp, dstIp, srcPort, dstPort, timestampNs);
    }
    ...
    public synchronized void onTcpSocketStatsEvent(int[] networkIds, int[] sentPackets, int[] lostPackets, int[] rttsUs, int[] sentAckDiffsMs) {
        if (networkIds.length == sentPackets.length && networkIds.length == lostPackets.length && networkIds.length == rttsUs.length && networkIds.length == sentAckDiffsMs.length) {
            ...
            for (int i = 0; i < networkIds.length; i++) {
                ...
                ITranNetdEventListenerService.Instance().onTcpSocketStatsEvent(netId, sent, lost, rttUs, sentAckDiffMs);
            }
            return;
        }
