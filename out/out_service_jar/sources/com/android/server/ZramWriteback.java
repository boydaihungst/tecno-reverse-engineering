...
public final class ZramWriteback extends JobService {
    ...
    public static void scheduleZramWriteback(Context context) {
        ...
        int firstWbDelay = SystemProperties.getInt(FIRST_WB_DELAY_PROP, (int) FrameworkStatsLog.DEVICE_POLICY_EVENT__EVENT_ID__CREDENTIAL_MANAGEMENT_APP_REQUEST_ACCEPTED);
        boolean forceWb = SystemProperties.getBoolean(FORCE_WRITEBACK_PROP, false);
        JobScheduler js = (JobScheduler) context.getSystemService("jobscheduler");
        ComponentName componentName = sZramWriteback;
        js.schedule(new JobInfo.Builder(MARK_IDLE_JOB_ID, componentName).setMinimumLatency(TimeUnit.MINUTES.toMillis(markIdleDelay)).setOverrideDeadline(TimeUnit.MINUTES.toMillis(markIdleDelay)).build());
        js.schedule(new JobInfo.Builder(WRITEBACK_IDLE_JOB_ID, componentName).setMinimumLatency(TimeUnit.MINUTES.toMillis(firstWbDelay)).setRequiresDeviceIdle(!forceWb).build());
    }
}
