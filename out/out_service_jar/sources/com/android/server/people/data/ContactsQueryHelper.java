...
class ContactsQueryHelper {
    ...
    private boolean queryContact(Uri uri, String[] projection, String selection, String[] selectionArgs) {
        ...
        try {
            ...
            Slog.w(TAG, "SQLException when querying contact", e1);
            return false;
        } catch (IllegalStateException e) {
            Slog.w(TAG, "IllegalStateException when querying contact", e);
            found = false;
        }
        if (cursor == null) {
    ...
    private boolean queryPhoneNumber(String lookupKey) {
        ...
        try {
            ...
            Slog.w(TAG, "SQLException when queryPhoneNumber", e1);
            return false;
        } catch (IllegalStateException e) {
            Slog.w(TAG, "IllegalStateException when queryPhoneNumber", e);
            return false;
        }
    }
}
