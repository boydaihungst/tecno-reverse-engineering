    ...
    public void storePassword(int userId, LockscreenCredential password) {
        synchronized (this.mEncryptedPasswords) {
            ...
            if (parent != null && parent.id == 0) {
                ...
                try {
                    KeyGenerator generator = KeyGenerator.getInstance("AES", this.mKeyStore.getProvider());
                    generator.init(new KeyGenParameterSpec.Builder(keyName, 3).setKeySize(256).setBlockModes("GCM").setNamespace(SyntheticPasswordCrypto.keyNamespace()).setEncryptionPaddings("NoPadding").setUserAuthenticationRequired(true).setUserAuthenticationValidityDurationSeconds(CACHE_TIMEOUT_SECONDS).build());
