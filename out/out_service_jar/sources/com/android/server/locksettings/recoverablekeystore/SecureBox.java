...
public class SecureBox {
    ...
    static {
        byte[] bArr = {2, 0};
        VERSION = bArr;
        HKDF_SALT = concat("SECUREBOX".getBytes(StandardCharsets.UTF_8), bArr);
        ...
        EllipticCurve curveSpec = new EllipticCurve(new ECFieldFp(bigInteger), subtract, bigInteger2);
    ...
    public static byte[] decrypt(PrivateKey ourPrivateKey, byte[] sharedSecret, byte[] header, byte[] encryptedPayload) throws NoSuchAlgorithmException, InvalidKeyException, AEADBadTagException {
        ...
        byte[] bArr = VERSION;
        byte[] version = readEncryptedPayload(ciphertextBuffer, bArr.length);
        if (!Arrays.equals(version, bArr)) {
    ...
    private static void validateEcPoint(BigInteger x, BigInteger y) throws InvalidKeyException {
        BigInteger bigInteger = EC_PARAM_P;
        if (x.compareTo(bigInteger) >= 0 || y.compareTo(bigInteger) >= 0 || x.signum() == -1 || y.signum() == -1) {
            throw new InvalidKeyException("Point lies outside of the expected curve");
        }
        BigInteger bigInteger2 = BIG_INT_02;
        BigInteger lhs = y.modPow(bigInteger2, bigInteger);
        BigInteger rhs = x.modPow(bigInteger2, bigInteger).add(EC_PARAM_A).mod(bigInteger).multiply(x).add(EC_PARAM_B).mod(bigInteger);
        if (!lhs.equals(rhs)) {
            throw new InvalidKeyException("Point lies outside of the expected curve");
        }
    }
