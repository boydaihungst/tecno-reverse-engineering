...
public class MtpNotificationManager {
    ...
    public void showNotification(UsbDevice device) {
        ...
        String title = resources.getString(17041674, device.getProductName());
        String description = resources.getString(17041673);
        Notification.Builder builder = new Notification.Builder(this.mContext, SystemNotificationChannels.USB).setContentTitle(title).setContentText(description).setSmallIcon(17303645).setCategory(Griffin.SYS);
        Intent intent = new Intent(ACTION_OPEN_IN_APPS);
        intent.putExtra("device", device);
        intent.addFlags(1342177280);
        PendingIntent openIntent = PendingIntent.getBroadcastAsUser(this.mContext, device.getDeviceId(), intent, AudioFormat.DTS_HD, UserHandle.ALL);
