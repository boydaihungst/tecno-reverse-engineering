...
public class AccessibilityWindowManager {
    ...
    public final class DisplayWindowsObserver implements WindowManagerInternal.WindowsForAccessibilityCallback {
        ...
        boolean computePartialInteractiveRegionForWindowLocked(int windowId, boolean forceComputeRegion, Region outRegion) {
            List<AccessibilityWindowInfo> list = this.mWindows;
            if (list == null) {
                return false;
            }
        ...
        private AccessibilityWindowInfo populateReportedWindowLocked(int userId, WindowInfo window) {
            ...
            if (AccessibilityWindowManager.this.mHostEmbeddedMap.size() > 0 && isEmbeddedHierarchyWindowsLocked(windowId)) {
                return null;
            }
        ...
        private boolean isEmbeddedHierarchyWindowsLocked(int windowId) {
            IBinder leashToken = (IBinder) AccessibilityWindowManager.this.mWindowIdMap.get(windowId);
            if (leashToken == null) {
                return false;
            }
            for (int i = 0; i < AccessibilityWindowManager.this.mHostEmbeddedMap.size(); i++) {
                if (((IBinder) AccessibilityWindowManager.this.mHostEmbeddedMap.keyAt(i)).equals(leashToken)) {
                    return true;
                }
            }
            return false;
        ...
        void dumpLocked(FileDescriptor fd, PrintWriter pw, String[] args) {
            ...
            pw.println("Top focused display Id = " + AccessibilityWindowManager.this.mTopFocusedDisplayId);
            pw.println("     Active Window Id = " + AccessibilityWindowManager.this.mActiveWindowId);
            pw.println("     Top Focused Window Id = " + AccessibilityWindowManager.this.mTopFocusedWindowId);
            pw.println("     Accessibility Focused Window Id = " + AccessibilityWindowManager.this.mAccessibilityFocusedWindowId + " ]");
            pw.println();
            List<AccessibilityWindowInfo> list = this.mWindows;
            if (list != null) {
                int windowCount = list.size();
                for (int j = 0; j < windowCount; j++) {
                    ...
                    pw.append(']');
                    pw.println();
                    WindowInfo windowInfo = findWindowInfoByIdLocked(window.getId());
                    if (windowInfo != null) {
                        pw.append("WindowInfo[");
                        pw.append((CharSequence) windowInfo.toString());
                        pw.append("]");
                        pw.println();
                    }
                }
    ...
    public AccessibilityWindowManager(Object lock, Handler handler, WindowManagerInternal windowManagerInternal, AccessibilityEventSender accessibilityEventSender, AccessibilitySecurityPolicy securityPolicy, AccessibilitySecurityPolicy.AccessibilityUserManager accessibilityUserManager, AccessibilityTraceManager traceManager) {
        this.mLock = lock;
    ...
    public boolean computePartialInteractiveRegionForWindowLocked(int windowId, Region outRegion) {
        ...
        DisplayWindowsObserver observer = getDisplayWindowObserverByWindowIdLocked(parentWindowId);
        if (observer != null) {
    ...
    private int findFocusedWindowId(int userId) {
        ...
        IBinder token = this.mWindowManagerInternal.getFocusedWindowTokenFromWindowStates();
        synchronized (this.mLock) {
    ...
    public void clearAccessibilityFocusLocked(int windowId) {
        ...
        }, this, Integer.valueOf(this.mAccessibilityUserManager.getCurrentUserIdLocked()), Integer.valueOf(windowId)));
    }
